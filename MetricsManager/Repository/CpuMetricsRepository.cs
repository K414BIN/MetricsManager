...
namespace MetricsManager.Repository
{
    public class CpuMetricsRepository 
    {
        ...
        public IList<CpuMetrics> GetAll()
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                return connection.Query<CpuMetrics>($"SELECT * FROM {_tblname}").ToList();
            }
        }

        public CpuMetrics GetById(int idIn)
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                return connection.QuerySingle<CpuMetrics>($"SELECT * FROM {_tblname} WHERE id=@id",
                    new {id = idIn});
            }
        }
