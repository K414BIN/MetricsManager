...
namespace MetricsManager.Repository
{
    public class DotNetMetricsRepository 
    {
        ...
        private string _tblname = "DotNetMetrics";
        ...
        public IList<DotNetMetrics> GetAll()
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                return connection.Query<DotNetMetrics>($"SELECT * FROM {_tblname}").ToList();
            }
        }

        public DotNetMetrics GetById(int idIn)
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                return connection.QuerySingle<DotNetMetrics>($"SELECT * FROM {_tblname} WHERE id=@id",
                    new {id = idIn});
            }
        }
